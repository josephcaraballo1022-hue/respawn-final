...
export default function Home() {
  ...
  // --- 👮‍♂️ ADMIN DASHBOARD & VERIFICACIÓN ---
  ...
  const [verified, setVerified] = useState(false) // 🔹 Estado Chulo Azul
  const [docFile, setDocFile] = useState(null)    // 🔹 Archivo Cédula
  ...
  const [duracionSubasta, setDuracionSubasta] = useState('24') // 🔹 Horas Subasta
  const [archivos, setArchivos] = useState([]) // 🔹 Galería Múltiple
  const [cargando, setCargando] = useState(false)
  const [estimado, setEstimado] = useState(null) // 🔹 Cálculo Comisión
  const [indicesGaleria, setIndicesGaleria] = useState({}) // Control carrusel
  ...
  useEffect(() => {
    ...
    .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'games' }, () => {
        cargarJuegosPaginados(0, true) // Actualizar si hay pujas
    })
    .subscribe()
      ...
      if (transacciones) {
          transacciones.forEach(t => {
              if(t.price > 0) { 
                  totalVentas += t.price
                  // Ganancia: 10% + 1000
                  totalGanancia += (t.price * 0.10) + 1000
              }
          })
      }
  ...
  async function aprobarVerificacion(userId, aprobar) {
      if(aprobar) {
          await supabase.from('profiles').update({ verified: true }).eq('id', userId)
          alert("Usuario Verificado ✅")
      } else {
          await supabase.from('profiles').update({ id_document_url: null }).eq('id', userId)
  ...
  async function solicitarVerificacion() {
      if(!docFile) return alert("Sube foto del documento")
      setUploadingDoc(true)
      try {
          const fName = `doc-${session.user.id}-${Date.now()}`
          await supabase.storage.from('imagenes').upload(fName, docFile)
          const { data } = supabase.storage.from('imagenes').getPublicUrl(fName)
          await supabase.from('profiles').update({ id_document_url: data.publicUrl }).eq('id', session.user.id)
          alert("Documento enviado. Espera aprobación.")
          setDocFile(null)
      } catch(e) { alert("Error al subir") } finally { setUploadingDoc(false) }
  }

  // --- 👤 PERFIL PÚBLICO ---
  async function abrirPerfilPublico(userId) {
      const { data: perfil } = await supabase.from('profiles').select('*').eq('id', userId).single()
      const { data: reviews } = await supabase.from('reviews').select('*, reviewer:profiles!reviewer_id(username, avatar_url)').eq('user_id', userId)
      const { count } = await supabase.from('games').select('*', { count: 'exact', head: true }).eq('user_id', userId).eq('condition', 'Vendido')
      const { data: catalogo } = await supabase.from('games').select('*, profiles(username, avatar_url)').eq('user_id', userId).neq('condition', 'Vendido')

      let promedio = 0
      if(reviews?.length) promedio = (reviews.reduce((a,b)=>a+b.stars,0)/reviews.length).toFixed(1)

      setPerfilPublico({ ...perfil, rating: promedio, totalVentas: count, totalReviews: reviews.length })
      setReviewsVendedor(reviews || [])
      setJuegosVendedor(catalogo || [])
      setModalPerfil(true)
  }

  // --- 📸 GALERÍA ---
  function cambiarImagen(gameId, direccion, total) {
      setIndicesGaleria(prev => {
          const actual = prev[gameId] || 0
          let nuevo = actual + direccion
          if (nuevo < 0) nuevo = total - 1
          if (nuevo >= total) nuevo = 0
          return { ...prev, [gameId]: nuevo }
      })
  }

  // --- 🔨 SUBASTAS ---
  async function realizarPuja(game) {
      if(!session) return alert("Inicia sesión")
      const ofertaMinima = (game.current_bid || game.price) + 1000
      const oferta = prompt(`Oferta actual: $${game.current_bid || game.price}\nTu oferta (mínimo $${ofertaMinima}):`)
      if(!oferta) return
      const monto = parseFloat(oferta)
      if(monto < ofertaMinima) return alert("Oferta muy baja")

      const { error } = await supabase.from('bids').insert([{ game_id: game.id, bidder_id: session.user.id, amount: monto }])
      if(!error) {
          await supabase.from('games').update({ current_bid: monto, highest_bidder_id: session.user.id }).eq('id', game.id)
  ...
  // --- 💰 CÁLCULO DE COMISIÓN ---
  function calcularDesglose(precioInput) {
      const p = parseFloat(precioInput)
      if (!p || p < 2000) return null
      const c = Math.round((p * 0.10) + 1000)
      return { comision: c, ganancia: p - c }
  }

  // --- LOGÍSTICA COMPRA ---
  function iniciarProcesoCompra(game) {
      if (game.type === 'subasta') {
          const finished = new Date(game.auction_end) <= new Date()
          if (!finished) return alert("Subasta activa. Espera que termine.")
          if (game.highest_bidder_id !== session.user.id) return alert("No eres el ganador.")
      }
      setJuegoAComprar(game); setModalCompra(true)
  }

  function procederAlPago() {
      if (datosEntrega.tipo === 'envio' && (!datosEntrega.direccion || !datosEntrega.telefono)) return alert("Faltan datos")
      setModalCompra(false)
      const precioFinal = juegoAComprar.type === 'subasta' ? juegoAComprar.current_bid : juegoAComprar.price
      const c = new WidgetCheckout({ currency: 'COP', amountInCents: precioFinal * 100, reference: `REF-${juegoAComprar.id}-${Date.now()}`, publicKey: WOMPI_PUBLIC_KEY, redirectUrl: window.location.href })
      c.open(async (r) => { if(r.transaction.status === 'APPROVED') await registrarCompraExitosa(juegoAComprar, precioFinal, r.transaction.id) })
  }

  async function registrarCompraExitosa(game, precio, ref) {
      const { data, error } = await supabase.from('transactions').insert([{ buyer_id: session.user.id, seller_id: game.user_id, game_id: game.id, price: precio, delivery_type: datosEntrega.tipo, shipping_address: datosEntrega.direccion, buyer_phone: datosEntrega.telefono }]).select().single()
      if (!error) {
          await supabase.from('games').update({ condition: 'Vendido' }).eq('id', game.id)
          setReciboData({ id: data.id, ref, game: game.title, price: precio, date: new Date().toLocaleString(), seller: game.profiles?.username, delivery: datosEntrega.tipo })
          setModalRecibo(true); cargarJuegosPaginados(0, true); cargarHistorialCompras(session.user.id)
      }
  }

  async function finalizarTrueque() {
      if(!confirm("¿Confirmar trueque?")) return
      await supabase.from('games').update({ condition: 'Vendido' }).eq('id', chatActivo.game_id)
      ...
      alert("Listo"); cargarJuegosPaginados(0, true); cargarMisChats(session.user.id)
  }

  // --- CORE FUNCTIONS (Upload Multiple, Pagination, etc) ---
  async function cargarJuegosPaginados(p, reset = false) {
    const f = p * PAGE_SIZE; 
    let q = supabase.from('games').select('*, profiles(username, avatar_url, verified), comments(count)')
    if (orden === 'recientes') q = q.order('id', { ascending: false })
    if (orden === 'precio_asc') q = q.order('price', { ascending: true })
    if (orden === 'precio_desc') q = q.order('price', { ascending: false })
    const { data, error } = await q.range(f, f + PAGE_SIZE - 1)
    if (!error) { 
        if (reset) setGames(data); else setGames(pr => [...pr, ...data]); 
        setHasMore(data.length >= PAGE_SIZE); setPage(p) 
    }
  }

  async function subirJuego(e) { 
      e.preventDefault(); if(!nuevoTitulo||archivos.length === 0)return alert("Faltan datos"); setCargando(true); 
      
      const urls = []
      for(const file of archivos) {
          const n = `${Date.now()}-${file.name}`
          await supabase.storage.from('imagenes').upload(n, file)
          const { data } = supabase.storage.from('imagenes').getPublicUrl(n)
          urls.push(data.publicUrl)
      }

      let auctionProps = {}
      if(nuevoTipo === 'subasta') {
          const end = new Date(); end.setHours(end.getHours() + parseInt(duracionSubasta))
          auctionProps = { auction_end: end, current_bid: nuevoPrecio }
      }

      await supabase.from('games').insert([{ 
          title:nuevoTitulo, platform:nuevaPlataforma, city:nuevaCiudad, genre:nuevoGenero, condition:'Disponible', type:nuevoTipo, 
          price: (nuevoTipo==='venta'||nuevoTipo==='subasta')?nuevoPrecio:0, 
          description:nuevaDescripcion, 
          image_url: urls[0], gallery: urls,
          user_id:session.user.id, ...auctionProps 
      }]); 
      
      setNuevoTitulo('');setArchivos([]); alert("Publicado"); cargarJuegosPaginados(0, true); setCargando(false) 
  }

  const TiempoRestante = ({ fin }) => {
    ...
    <div className="min-h-screen font-sans text-white relative selection:bg-green-500 selection:text-black" style={{ backgroundImage: "url('/fondo.png')", backgroundAttachment: 'fixed', backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <div className="relative z-10 p-4 sm:p-8">
          ...
          {modalPerfil && perfilPublico && (
              <div className="fixed inset-0 bg-black/90 z-[80] overflow-y-auto p-4 flex justify-center">
                  <div className={`${cardStyle} max-w-4xl w-full p-6 relative h-fit mt-10`}>
                      <button onClick={() => setModalPerfil(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white text-xl">✕</button>
                      <div className="flex flex-col items-center border-b border-gray-700 pb-6">
                          <img src={perfilPublico.avatar_url || "https://via.placeholder.com/150"} className="w-24 h-24 rounded-full border-4 border-blue-500/50 mb-3 object-cover"/>
                          <h2 className="text-2xl font-bold flex items-center gap-2">{perfilPublico.username} {perfilPublico.verified && <span title="Verificado">🔵</span>}</h2>
                          <p className="text-gray-400 italic text-sm mt-1">"{perfilPublico.bio || 'Sin biografía'}"</p>
                          <div className="flex gap-4 mt-4 text-sm"><span className="text-yellow-400 font-bold">⭐ {perfilPublico.rating} Reputación</span><span className="text-green-400 font-bold">🛒 {perfilPublico.totalVentas} Ventas</span></div>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
                          <div><h3 className="text-lg font-bold text-blue-400 mb-4 uppercase">Catálogo en Venta</h3><div className="space-y-3">{juegosVendedor.length > 0 ? juegosVendedor.map(g => (<div key={g.id} className="flex gap-3 bg-gray-800/50 p-2 rounded cursor-pointer hover:bg-gray-700" onClick={() => { setModalPerfil(false); iniciarProcesoCompra(g) }}><img src={g.image_url} className="w-12 h-12 rounded object-cover"/><div className="flex-1"><p className="font-bold text-sm text-white">{g.title}</p><p className="text-xs text-green-400">${new Intl.NumberFormat('es-CO').format(g.price)}</p></div></div>)) : <p className="text-gray-500 text-xs">No tiene juegos activos.</p>}</div></div>
                          <div><h3 className="text-lg font-bold text-yellow-400 mb-4 uppercase">Opiniones ({perfilPublico.totalReviews})</h3><div className="max-h-60 overflow-y-auto space-y-3 custom-scrollbar">{reviewsVendedor.map(r => (<div key={r.id} className="bg-gray-800/50 p-3 rounded text-sm"><div className="flex justify-between mb-1"><span className="font-bold text-blue-300">{r.reviewer?.username}</span><span className="text-yellow-400">{'⭐'.repeat(r.stars)}</span></div><p className="text-gray-300 italic">"{r.comment}"</p></div>))}</div></div>
                      </div>
                  </div>
              </div>
          )}
          ...
          {vista === 'admin' && session && session.user.email === ADMIN_EMAIL && (
              <div className="max-w-6xl mx-auto space-y-6">
                  ...
                  <div className={`grid grid-cols-1 md:grid-cols-3 gap-6`}>
                      <div className={`${cardStyle} p-6 flex flex-col items-center justify-center bg-blue-900/20 border-blue-500/30`}>
                          <h3 className="text-gray-400 text-xs uppercase tracking-widest mb-1">Volumen Ventas</h3>
                          <p className="text-3xl font-bold text-blue-400">${new Intl.NumberFormat('es-CO').format(stats.ventas)}</p>
                      </div>
                      <div className={`${cardStyle} p-6 flex flex-col items-center justify-center bg-green-900/20 border-green-500/30`}>
                          <h3 className="text-gray-400 text-xs uppercase tracking-widest mb-1">Tu Ganancia</h3>
                          <p className="text-3xl font-bold text-green-400">+${new Intl.NumberFormat('es-CO').format(stats.ganancia)}</p>
                      </div>
                      <div className={`${cardStyle} p-6 flex flex-col items-center justify-center`}>
                          <h3 className="text-gray-400 text-xs uppercase tracking-widest mb-1">Usuarios</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className={`p-6 ${cardStyle}`}>
                          <h3 className="font-bold mb-4 text-yellow-400 text-lg uppercase tracking-wider">🆔 Verificaciones ({listaVerificaciones.length})</h3>
                          <div className="max-h-96 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                              {listaVerificaciones.map(u => (
                                  <div key={u.id} className="bg-gray-800/50 p-3 rounded border border-gray-700">
                                      <p className="text-white font-bold mb-2">{u.username}</p>
                                      <img src={u.id_document_url} className="w-full h-32 object-cover rounded mb-2 border border-gray-600"/>
                                      <div className="flex gap-2">
                                          <button onClick={() => aprobarVerificacion(u.id, true)} className="flex-1 bg-green-600 hover:bg-green-500 text-white text-xs font-bold py-2 rounded">APROBAR ✅</button>
                                          <button onClick={() => aprobarVerificacion(u.id, false)} className="flex-1 bg-red-600 hover:bg-red-500 text-white text-xs font-bold py-2 rounded">RECHAZAR ❌</button>
                                      </div>
                                  </div>
                              ))}
                              {listaVerificaciones.length === 0 && <p className="text-gray-500 italic">Todo al día.</p>}
                          </div>
                      </div>
                      <div className={`p-6 ${cardStyle}`}>
                          <h3 className="font-bold mb-4 text-red-400 text-lg uppercase tracking-wider">🚨 Reportes Activos</h3>
                          <div className="max-h-96 overflow-y-auto space-y-2 pr-2 custom-scrollbar">{listaReportes.map(r => (<div key={r.id} className="bg-red-900/20 p-3 rounded border border-red-500/30 hover:border-red-500 transition-colors"><p className="text-red-300 text-xs font-bold uppercase">{r.profiles?.username} reportó:</p><p className="text-white text-sm font-bold mt-1">{r.games?.title}</p><p className="text-gray-300 text-xs mt-1 italic">"{r.reason}"</p><button onClick={() => adminBorrarJuego(r.game_id)} className="mt-3 w-full bg-red-600 hover:bg-red-500 text-white text-xs font-bold py-2 rounded shadow-lg">⚠️ ELIMINAR CONTENIDO</button></div>))}{listaReportes.length===0 && <p className="text-gray-500 italic">Sin reportes.</p>}</div>
                      </div>
                  </div>
              </div>
          )}
                  ...
                  <div className={`p-6 ${cardStyle}`}>
                      ...
                      <div className="flex flex-col items-center mb-6">
                          ...
                          <div className="flex items-center gap-1 mb-2">
                              <input type="text" placeholder="GamerTag" value={username} onChange={e=>setUsername(e.target.value)} className="w-full bg-transparent text-center text-2xl font-bold text-white border-b-2 border-gray-700 focus:border-blue-500 outline-none"/>
                              {verified && <span className="text-xl" title="Verificado">🔵</span>}
                          </div>
                          {!verified && (
                              <div className="w-full mb-4 p-3 bg-gray-800/50 rounded border border-gray-700">
                                  <p className="text-xs text-gray-400 mb-2 text-center">Verifica tu identidad para obtener el chulo azul 🔵</p>
                                  <label className="block w-full bg-blue-900/30 hover:bg-blue-900/50 text-blue-300 text-center text-xs py-2 rounded cursor-pointer transition-colors border border-blue-500/30 border-dashed">
                                      {docFile ? docFile.name : "📷 Subir Documento"}
                                      <input type="file" className="hidden" accept="image/*" onChange={e => setDocFile(e.target.files[0])} />
                                  </label>
                                  {docFile && <button onClick={solicitarVerificacion} disabled={uploadingDoc} className="w-full mt-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold py-2 rounded">{uploadingDoc ? "Enviando..." : "Enviar Solicitud"}</button>}
                              </div>
                          )}
          ...
          {vista === 'mercado' && (
            <>
              ...
              <div className={`p-4 mb-8 flex flex-col md:flex-row gap-4 items-center sticky top-24 z-40 ${cardStyle}`}>
                 ...
                 <select className="p-3 bg-gray-900/50 border border-gray-700 rounded-lg text-white outline-none focus:border-green-500 md:w-28 cursor-pointer hover:bg-gray-800 transition-colors" value={filtroPlataforma} onChange={e=>setFiltroPlataforma(e.target.value)}><option value="">Plataforma</option><option value="PS5">PS5</option><option value="Switch">Switch</option><option value="Xbox">Xbox</option><option value="PC">PC</option></select>
                 <select className="p-3 bg-gray-900/50 border border-gray-700 rounded-lg text-white outline-none focus:border-green-500 md:w-28 cursor-pointer hover:bg-gray-800 transition-colors" value={filtroCiudad} onChange={e=>setFiltroCiudad(e.target.value)}><option value="">Ciudad</option>{CIUDADES_COLOMBIA.map(c => <option key={c} value={c}>{c}</option>)}</select>
                 <select className="p-3 bg-gray-900/50 border border-gray-700 rounded-lg text-white outline-none focus:border-green-500 md:w-28 cursor-pointer hover:bg-gray-800 transition-colors" value={filtroGenero} onChange={e=>setFiltroGenero(e.target.value)}><option value="">Género</option>{GENEROS_JUEGOS.map(g => <option key={g} value={g}>{g}</option>)}</select>
                ...
                <form onSubmit={subirJuego} className="flex flex-col gap-4">
                  <div className="border-2 border-dashed border-gray-600 rounded-xl p-6 text-center cursor-pointer hover:border-green-500 hover:bg-green-500/5 transition-all group/file">
                      <label className="cursor-pointer block">
                          ...
                          <p className="text-sm text-gray-400 group-hover/file:text-white">{archivos.length > 0 ? `${archivos.length} fotos seleccionadas` : "Arrastra o selecciona hasta 4 fotos"}</p>
                  ...
                  {nuevoTipo === 'venta' && (
                      <div className="space-y-2">
                          <input type="number" placeholder="Precio ($)" className={`${inputStyle} border-green-500/50 bg-green-900/20`} value={nuevoPrecio} onChange={e => {setNuevoPrecio(e.target.value);setEstimado(calcularDesglose(e.target.value))}}/>
                          {estimado && (<div className="flex justify-between items-center text-xs bg-black/40 p-2 rounded border border-gray-700 animate-fade-in"><span className="text-red-400">Comisión: -${new Intl.NumberFormat('es-CO').format(estimado.comision)}</span><span className="text-green-400 font-bold">💰 RECIBES: ${new Intl.NumberFormat('es-CO').format(estimado.ganancia)}</span></div>)}
                      </div>
                  )}
                  {nuevoTipo === 'subasta' && (
                      <div className="space-y-2">
                          <div className="grid grid-cols-2 gap-4">
                              <input type="number" placeholder="Precio Base ($)" className={`${inputStyle} border-purple-500/50 bg-purple-900/20`} value={nuevoPrecio} onChange={e=>setNuevoPrecio(e.target.value)}/>
                              <select className={inputStyle} value={duracionSubasta} onChange={e=>setDuracionSubasta(e.target.value)}><option value="12">12 Horas</option><option value="24">24 Horas</option><option value="48">48 Horas</option></select>
                          </div>
                          <p className="text-xs text-purple-400 text-center">⏳ La subasta iniciará inmediatamente.</p>
                      </div>
                  )}
                ...
                {juegosFiltrados.map((game) => {
                    ...
                    return (
                        <div key={game.id} className={`group relative overflow-hidden flex flex-col ${cardStyle}`}>
                            ...
                            {/* IMAGEN Y CARRUSEL */}
                            <div className="h-56 bg-gray-900 relative overflow-hidden group/img">
                                <img src={imagenActual} className="w-full h-full object-cover transition-transform duration-500 group-hover/img:scale-110" />
                                {game.gallery?.length > 1 && (
                                    <>
                                        <button onClick={(e) => {e.stopPropagation(); cambiarImagen(game.id, -1, game.gallery.length)}} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 p-2 rounded-full hover:bg-black/80">‹</button>
                                        <button onClick={(e) => {e.stopPropagation(); cambiarImagen(game.id, 1, game.gallery.length)}} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 p-2 rounded-full hover:bg-black/80">›</button>
                                    </>
                                )}
                                <div className="absolute top-3 left-3 flex flex-col gap-1 items-start"><span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider shadow-lg ${game.type === 'venta' ? 'bg-blue-600 text-white' : (game.type === 'subasta' ? 'bg-purple-600 text-white' : 'bg-orange-600 text-white')}`}>{game.type}</span><span className="bg-black/60 backdrop-blur-sm px-2 py-1 rounded text-[10px] text-white flex items-center gap-1 border border-white/10">📍 {game.city}</span></div>{game.condition === 'Reservado' && <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] flex items-center justify-center z-10"><span className="bg-yellow-500 text-black font-extrabold px-6 py-2 rounded-lg rotate-[-12deg] shadow-2xl border-4 border-yellow-300 text-xl">RESERVADO</span></div>}
                            </div>
                                ...
                                <div className="mt-auto space-y-3">
                                    ...
                                    {game.type === 'subasta' ? (
                                        <div className="bg-purple-900/30 border border-purple-500/30 p-2 rounded text-center">
                                            <p className="text-xs text-purple-300">Oferta Actual:</p>
                                            <p className="text-lg font-bold text-white">${new Intl.NumberFormat('es-CO').format(game.current_bid || game.price)}</p>
                                            <div className="text-xs mt-1"><TiempoRestante fin={game.auction_end} /></div>
                                            {session && new Date(game.auction_end) > new Date() && (<button onClick={() => realizarPuja(game)} className="w-full mt-2 bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold py-2 rounded">🔨 OFERTAR</button>)}
                                            {session && new Date(game.auction_end) <= new Date() && game.highest_bidder_id === session.user.id && (<button onClick={() => iniciarProcesoCompra(game)} className="w-full mt-2 bg-green-600 hover:bg-green-500 text-white text-xs font-bold py-2 rounded animate-pulse">👑 ¡GANASTE! PAGAR</button>)}
                                        </div>
                                    ) : (
                                        session ? (
