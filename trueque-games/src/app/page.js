...
export default function Home() {
  ...
  const PAGE_SIZE = 8 // Cargar de 8 en 8

  // --- 🏅 GAMIFICACIÓN & HISTORIAL ---
  const [misCompras, setMisCompras] = useState([])
  const [ventasCount, setVentasCount] = useState(0) // Para medalla "Vendedor"
  ...
  const WOMPI_PUBLIC_KEY = "pub_test_Q5yDA9N9OyJSaxZZwz48e5r195Tz01E9"
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      else cargarJuegosPaginados(0, true) // Carga inicial
    })
  ...
  async function inicializarDatos(userId) {
    ...
    await cargarHistorialCompras(userId) // <--- NUEVO
    if (session?.user?.email === ADMIN_EMAIL) cargarReportes()
  }

  // --- ⚡ FUNCIÓN DE CARGA PAGINADA ---
  async function cargarJuegosPaginados(pagina, reset = false) {
    ...
    if (!error) {
        ...
        // Si trajo menos de lo que pedimos, no hay más
  ...
  async function cargarHistorialCompras(userId) {
      const { data } = await supabase.from('transactions')
        .select('*, games(title, image_url)')
        .eq('buyer_id', userId)
        .order('created_at', { ascending: false })
      if(data) setMisCompras(data)
  }

  // --- 💳 PAGO WOMPI + REGISTRO TRANSACCIÓN ---
  const handleWompiPayment = (game) => {
    const checkout = new WidgetCheckout({ currency: 'COP', amountInCents: game.price * 100, reference: `REF-${game.id}-${Date.now()}`, publicKey: WOMPI_PUBLIC_KEY, redirectUrl: window.location.href });
    ...
    checkout.open(async (result) => {
      if(result.transaction.status === 'APPROVED'){
        ...
        alert("¡Compra Exitosa! Guardada en tu historial.")
        cargarJuegosPaginados(0, true) // Recargar mercado
      }
    })
  }
  ...
  function calcularMedallas() {
      const medallas = [
          { icon: '🔰', nombre: 'Novato', desc: 'Bienvenido a la grieta', activo: true },
          { icon: '🤝', nombre: 'Negociador', desc: 'Completó una reseña', activo: totalReviews > 0 },
          { icon: '💎', nombre: 'Vendedor Pro', desc: '3+ Ventas realizadas', activo: ventasCount >= 3 },
          { icon: '⭐', nombre: 'Legendario', desc: '5 Estrellas de Reputación', activo: ratingPromedio >= 4.8 }
      ]
      return medallas
  }
  ...
  async function obtenerPerfil(uid) { 
      ...
      const {data:r}=await supabase.from('reviews').select('stars').eq('user_id',uid); 
      if(r?.length){ setRatingPromedio((r.reduce((a,b)=>a+b.stars,0)/r.length).toFixed(1)); setTotalReviews(r.length) }
      // Contar ventas para medalla
      const {count} = await supabase.from('games').select('*', {count: 'exact', head: true}).eq('user_id', uid).eq('condition', 'Vendido');
  ...
  return (
    <div className="min-h-screen font-sans text-white relative selection:bg-green-500 selection:text-black" style={{ backgroundImage: "url('/fondo.png')", backgroundAttachment: 'fixed', backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <div className="relative z-10 p-4 sm:p-8">
          ...
          {/* PERFIL (CON MEDALLAS Y MIS COMPRAS) */}
          {vista === 'perfil' && session && (
            <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="border-t border-gray-700 pt-4">
                          ...
                          <div className="flex justify-center gap-2 flex-wrap">
                              {calcularMedallas().map((medalla, i) => (
                                  <div key={i} title={medalla.desc} className={`w-10 h-10 rounded-full flex items-center justify-center text-xl cursor-help transition-all ${medalla.activo ? 'bg-yellow-500/20 border border-yellow-500 shadow-[0_0_10px_rgba(234,179,8,0.5)]' : 'bg-gray-800 grayscale opacity-30'}`}>
                                      {medalla.icon}
                                  </div>
                              ))}
                          </div>
                ...
                <div className="md:col-span-2 space-y-8">
                  {/* PESTAÑAS (INVENTARIO / COMPRAS / NOTIFICACIONES) */}
                  ...
                  <div className={`${cardStyle} p-6`}>
                      ...
                      {misCompras.length > 0 ? (
                          ...
                          <p className="text-gray-500 italic">Aún no has comprado nada. ¡Ve al mercado!</p>
                      )}
                  </div>
            ...
            <>
              ...
              {hasMore && (
                  <div className="flex justify-center pb-20">
                      <button onClick={() => cargarJuegosPaginados(page + 1)} className="bg-gray-800 border border-gray-600 hover:bg-gray-700 text-white font-bold py-3 px-10 rounded-full shadow-lg transition-transform hover:scale-105 flex items-center gap-2">
                          ⚡ CARGAR MÁS MISIONES
                      </button>
                  </div>
              )}
